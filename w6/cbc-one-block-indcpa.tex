...
\begin{tikzpicture}[yscale=-1,xscale=0.8,>=latex]
		...
		\node[message] (mOnea) at  	(7,  0) {$m_b$};
		\node[XOR] (xOnea) at 		(7, 0.85) {};
		\node[iv] (iOnea) at 		(2.5, 0.85) {$IV$};
		\node[key] (kOnea) at 		(5, 2) {$k$};
		\node[prf] (pOnea) at		(7, 2) {$F$};
		\node[tag] (cOnea) at		(7, 4) {$F_k(m_b \oplus c_0)$};
		\node[tag] (cZera) at		(2.5,4) {$c_0$};
		\draw[->, thick] (mOnea.south) -- (xOnea.north);
		\draw[->, thick] (xOnea.south) -- (pOnea.north);
		\draw[->, thick] (iOnea.east) -- (xOnea.west);
		\draw[->, thick] (kOnea.east) -- (pOnea.west);
		\draw[->,thick] (pOnea.south) -- (cOnea.north); 
		\draw[->,thick] (iOnea.south) -- (cZera.north);
