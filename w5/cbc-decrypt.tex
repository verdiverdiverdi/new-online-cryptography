...
\begin{tikzpicture}[yscale=-1,xscale=0.8,>=latex]
		...
		\node[message] (mThree) at  (9, 0) {$m_3$};
		\node[key] (kThree) at 		(7, 2) {$k$};
		\node[prf] (pThree) at		(9, 2) {$F^{-1}$};
		\node[tag] (cThree) at		(9, 4) {$c_3$};
		\draw[<-, thick] (mThree.south) -- (xThree.north);
