...
\begin{tikzpicture}[yscale=-1,xscale=0.8,>=latex]
		...
		\node[nonce] (N) at (-4.5,0) {$N$};
		\node[tag] (cZer) at (-4.5,4) {$c_0$};
		\draw[->,thick] (N.south) -- (cZer.north);

		\node[message] (mOne) at  	(-2, 3) {$m_1$};
		\node[nonce] (NOne) at (0,0)		{$N$};
		\node[XOR] (xOne) at (0,3)			{};
