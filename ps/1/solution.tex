...
\begin{document}
    ...
    $i$. In \S 1.9 we introduced notions of `confidentiality' and `alt-confidentiality'.

	Give an example of partial information an adversary might learn about a message from observing its ciphertext, short of the learning the message entirely, that would prevent you from calling the encryption scheme secure.
	...
	\solution{%
		...
		This may seem like a contrived example, but it serves to indicate that we need a way to reason about ciphertexts revealing (almost) no information, independent of context -- an encryption scheme should be universally useable; not depending on the messages it will be used to encrypt, or the situation in which it will be used.
	}
	
	$ii$. Can you think of a further example where `less' partial information is learnt by an adversary than your answer to ($i$.)?
    ...
    \solution{%
		Suppose there is some property of a message (some `information' about it) that an adversary can determine from the ciphertext.
		This might be, as in the above, the first two letters of its decryption, or the number of times a $1$ appears.
		The adversary selects two messages that differ in this particular way, e.g. AA and AB or $00000$ and $11111$ and it will succeed in knowing which of the messages it submitted were encrypted.
